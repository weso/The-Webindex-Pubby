	...
	protected boolean doGet(MappedResource resource,
		...
		while (it.hasNext()) {
			...
			if (!stmt.getSubject().isAnon())
				continue;
			String pathDataURL = resource.getInversePathDataURL(stmt
					.getPredicate());
			((Resource) stmt.getSubject().as(Resource.class)).addProperty(
					RDFS.seeAlso, description.createResource(pathDataURL));
		}
		...
		Resource document = description.getResource(addQueryString(
				resource.getDataURL(), request));
		...
		resource.getDataset().addMetadataFromTemplate(description, resource,
				getServletContext());
